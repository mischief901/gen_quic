...
timer_init(Data) ->
  ...
  Data#quic_data{timer_info = Timer}.
...
start_pacing_timer(#quic_data{
  ...
  Data0#quic_data{timer_info = Time_Info}.
...
available_packet_size(#quic_data{
                         cc_info = #{
...
packet_sent(_, 
  ...
  Data = Data0#quic_data{timer_info = Timer_Info,
                         cc_info = CC},
  {ok, Data}.
...
handle_acks(CC_State0,
  ...
  Data1 = Data0#quic_data{timer_info = Timer_Info},
...
has_congestion_event(_,
                     #quic_data{cc_info = #{ecn_ce_counter := _Total} = CC0} = Data0, 
                     Largest_Pkt_Num,
                     {ECT0, ECT1, ECN_Total}) when _Total < ECN_Total ->
  %% There is ecn info in the ack frame and the ecn count is greater
  Data = Data0#quic_data{cc_info = CC0#{ecn_ce_counter => ECN_Total}},
...
check_packet_loss(CC_State, #quic_data{timer_info = 
                                         #{sent_packets := Sent,
...
handle_ack_cc(recovery,
              #quic_data{cc_info = #{end_of_recovery := End,
                                     congestion_window := Window
                                    } = CC_Info0} = Data0, 
              [{Pkt_Num, {_, Size, _}} | Rest],
              Total) when Pkt_Num > End ->
  %% End of recovery. Switch CC_State to Slow_Start.
  Data = Data0#quic_data{cc_info = CC_Info0#{end_of_recovery => infinity,
...
handle_ack_cc(slow_start,
              #quic_data{cc_info = #{ss_threshold := Threshold,
                                     congestion_window := Window
                                    } = CC_Info0
                        } = Data0,
              [{Pkt_Num, {_, Size, _}} | Rest], Total) when Window < Threshold ->
  %% Stay in slow_start. Increase the congestion window
  Data = Data0#quic_data{cc_info = CC_Info0#{congestion_window => Window + Size}},
  handle_ack_cc(slow_start, Data, Rest, Total + Size);

handle_ack_cc(slow_start,
              #quic_data{cc_info = #{congestion_window := Window0,
  ...
  Data = Data0#quic_data{cc_info = CC_Info0#{congestion_window => Window}},
...
decrease_bytes_in_flight(#quic_data{cc_info = #{bytes_in_flight := In_Flight} = CC0} = Data0,
                         Total_Acked) ->
  Data0#quic_data{cc_info = CC0#{bytes_in_flight => In_Flight - Total_Acked}}.
...
packet_lost(#quic_data{timer_info = #{sent_packets := Sent0} = Timer0} = Data0,
            Pkt_Nums) ->
  ...
  Data1 = Data0#quic_data{timer_info = #{sent_packets => Sent}},
...
congestion_event(#quic_data{cc_info = #{congestion_window := Window0,
  ...
  Data = Data0#quic_data{cc_info = CC0#{congestion_window => Window,
